...
CI_single(s1 =sd1[,7], replicates = 3 , alpha=0.01)
##Global critical interval for 2 protein sets
CI_2pts(s1 = sd1[,7], s2=sd1[,8], replicates = 3, alpha=0.01)
...
###All peptides drawn 
robot_plot_All(thP = tc_aP, th=tc_a, pv_cutoff = 0.01,alpha=0.01)
###USe more stingent parameters to have less peptides drawn. alpha modifies the CI values (works similarly to p-value)
robot_plot_All(thP = tc_aP, th=tc_a, pv_cutoff = 0.001, alpha=0.01)
...
## returns dataframe with all significant peptides with pvalue=0.01, and alpha of 0.001 
robot_indexes_df(thP = tc_aP, th=tc_a, states=rev(names_states)[1:2], 
                    pvalue = 0.005, alpha=0.001)

#list of indexes 
inds<-robot_indexes(thP = tc_aP, th=tc_a, states=rev(names_states)[1:2], 
                    pvalue = 0.005, alpha=0.01) ###return indexes of peptide

inds2=inds[seq(1,16, by=2)] ### pick which peptides you want to keep 
...
robot_2states_indexes(thP = tc_aP, th=tc_a, states=rev(names_states)[1:2],indexes = inds2, pvalue=0.001, alpha=0.001, ylim=c(-120, 120), xlim=c(50, 230))
...
###USe more stingent parameters to have less peptides drawn. alpha modifies the Critial interval. 
woods_CI_plot(thP = tc_aP, th=tc_a, pv_cutoff=0.005, alpha=0.001)
...
pymol_script_significant_peptide_proc(input_proc = b, input_up = a, path=tempdir(), ranges=c(-Inf, seq(-120, 120, by=10), Inf), alpha=0.005, pv_cutoff = 0.005,replicates = 3)

###scripts prepared by residue
pymol_script_significant_residue(a,  ranges=c(-Inf, seq(-120, 120, by=10), Inf), path=tempdir(), alpha=0.005,pv_cutoff = 0.005,replicates = 3)
...
all_summary(file_nm, Dfact=0.85,replicates = 3, alpha=0.01)
